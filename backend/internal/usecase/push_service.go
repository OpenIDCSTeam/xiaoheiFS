...
func NewPushService(tokens PushTokenRepository, users UserRepository, settings SettingsRepository, sender PushSender) *PushService {
	return &PushService{
		tokens:   tokens,
		users:    users,
		settings: settings,
...
func (s *PushService) RegisterToken(ctx context.Context, userID int64, platform, token, deviceID string) error {
	if s.tokens == nil {
...
func (s *PushService) RemoveToken(ctx context.Context, userID int64, token string) error {
	if s.tokens == nil {
		return ErrInvalidInput
	}
	if userID == 0 {
...
func (s *PushService) NotifyAdminsNewOrder(ctx context.Context, order domain.Order) error {
