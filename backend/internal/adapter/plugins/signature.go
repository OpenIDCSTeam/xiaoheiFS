...
func VerifySignature(pluginDir string, officialKeys []ed25519.PublicKey) (domain.PluginSignatureStatus, error) {
	...
	if strings.ToLower(strings.TrimSpace(cs.Algo)) != "sha256" {
		return domain.PluginSignatureUntrusted, errors.New("unsupported checksum algo")
	}
	if len(cs.Files) == 0 {
		return domain.PluginSignatureUntrusted, errors.New("empty checksums")
	}
	...
	for rel, wantHex := range cs.Files {
		...
		if rel == "" {
			return domain.PluginSignatureUntrusted, errors.New("invalid checksums path")
		}
		...
		if len(wantHex) != 64 {
			return domain.PluginSignatureUntrusted, errors.New("invalid checksum")
		}
		...
		if wantHex != sum {
			return domain.PluginSignatureUntrusted, errors.New("checksum mismatch")
		}
	}
	...
	if len(sigBytes) != ed25519.SignatureSize {
		...
		if derr != nil || len(decoded) != ed25519.SignatureSize {
			return domain.PluginSignatureUntrusted, errors.New("invalid signature")
		}
		sigBytes = decoded
	}
...
func ensureBinFilesCovered(pluginDir string, files map[string]string) error {
	...
	if !st.IsDir() {
		return errors.New("bin is not a directory")
	}
	return filepath.WalkDir(binDir, func(path string, d os.DirEntry, err error) error {
		...
		if _, ok := files[rel]; !ok {
			return errors.New("checksums missing for bin file: " + rel)
		}
		return nil
	})
}
